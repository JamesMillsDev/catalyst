...
map<string, string> DATA_LAYOUTS =
{
	{ "Position", "0" },
	{ "Normal", "1" },
	{ "TexCoord0", "2" },
...
namespace Catalyst
{
	...
	bool ShaderParser::ScanForVertexStruct(const string& _line, const int& _currentLineIndex, bool& _inStruct, pair<int, int>& _structIndexes, const string& _structTitle) const
	{
	...
	Shader::SubShader* ShaderParser::ReadShader(const vector<string>& _file, const Shader::EShaderStage& _stage)
	{
		...
		glfwSetClipboardString(glfwGetCurrentContext(), code.c_str());
	...
	string ShaderParser::InjectVertexStructures(const vector<string>& _file, const string& _code, const bool _injectInput) const
	{
		...
		if (_injectInput)
		{
			for (int i = m_inputLocations.first - 1; i < m_inputLocations.second; ++i)
			{
				...
				string layout = DATA_LAYOUTS[varName];
