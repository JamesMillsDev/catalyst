...
namespace Catalyst
{
	...
	bool ActorTransform::IsChild(ActorTransform* _other, ActorTransform* _next)
	{
		for (auto& child : m_children)
		{
			if (!child->Children().empty())
			{
				if (IsChild(_other, child))
				{
					return true;
				}
			}
	...
	ActorTransform::ActorTransform(Actor* _owner)
		: m_parent{ nullptr }, m_transform{ new Matrix4(1.f) }, m_owner{ _owner }
	{
	}
	...
	void ActorTransform::RemoveChild(ActorTransform* _child)
	{
		...
		if (_child->m_parent == this)
		{
			...
			list<ActorTransform*>::iterator iter = std::ranges::find(m_children, _child);
	...
	void ActorTransform::Unparent(ActorTransform* _child)
	{
		if (std::ranges::find(m_children, _child) == m_children.end())
		{
			return;
