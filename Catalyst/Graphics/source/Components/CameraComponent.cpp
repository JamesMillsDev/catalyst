...
namespace Catalyst
{
    ...
    mat4 CameraComponent::ProjectTransform(const mat4& _mat)
    {
        return ProjectionView() * _mat;
    }

    mat4 CameraComponent::ProjectionView()
    {
        if (const ActorTransform* transform = Owner()->Transform())
        {
            m_viewMat = lookAt(transform->Location(), transform->Location() + transform->Forward(), transform->Up());
