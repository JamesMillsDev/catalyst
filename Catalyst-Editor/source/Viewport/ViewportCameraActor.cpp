...
namespace Catalyst
{
	...
	void ViewportCameraActor::Tick()
	{
		...
		if (const ActorTransform* transform = Transform())
		{
			...
			m_speedFactor = glm::clamp(m_speedFactor, .1f, 2.5f);
	...
	void ViewportCameraActor::ConfineMouse()
	{
		...
		if (cursorX > width - 1)
		{
			glfwSetCursorPos(context, 0, cursorY);
		}
		else if (cursorX < 1)
		{
			glfwSetCursorPos(context, width, cursorY);
		}
		if (cursorY > height - 1)
		{
			glfwSetCursorPos(context, cursorX, 0);
		}
		else if (cursorY < 1)
		{
			glfwSetCursorPos(context, cursorX, height);
		}
	}
